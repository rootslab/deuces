...
exports.Deuces = ( function () {
        ...
        , Deuces = function ( opt ) {
            ...
            var cfg = improve( clone( opt ), deuces_opt )
                ...
                , processCommandQueue = function ( address ) {
                    if ( gerry.list.length ) me.emit( 'scanqueue', bilanx.cqueue.size() );
                ...
                , send = function ( ocmd ) {
                    ...
                    if ( ~ bilanx.push( ocmd ) ) {
                        ...
                        return me.ready ?
                               ...
                               gerry.list.length ? me.emit( 'queued', ocmd, bilanx.cqueue.size() ) : null
                               ;
                    }
    ...
    dproto.cli = function ( on, fn ) {
        var me = this
            , mfn = on === undefined || on === null ? 'enable' : !! on ? 'enable' : 'disable'
            ;
        return me.cli_events[ mfn ]( fn );
    };

    dproto.initTasks = function ( list ) {
        var me = this
            ;
